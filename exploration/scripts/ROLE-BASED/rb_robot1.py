...
def share_data(robot_i):
	...
	print("Robot1 connected to ROBOT"+ str(robot_i)+ " - Sending map and goal position...")
	i=0
	while i<2:
		if not grid== None:
...
def check_connected_robots():
	...
	connection_list=(rospy.get_param("/connection_list_robot1"))
...
def check_nearest_robot_to_base(other_bot_goal_x, other_bot_goal_y):
	...
	if my_distance <= other_distance:
		#print("Not connected, closest bot to the BS. Need to go back to the BS!")
		going_to_base = 1
...
def callback_map(mess):
	...
	if mess.command == "map":
		...
		if not len(base_map)==0:
			...
			diff = np.array(merged_map_np, dtype = np.int8)
			diff = merged_map_np-base_map_np
			#number of new elements
			new_elements = np.count_nonzero(diff)
