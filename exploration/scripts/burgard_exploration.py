...
def odom_callback(odom_data):
	global robot_x,robot_y, covered_distance, starting_time;
	...
	if not starting_time == None:
		...
		if abs(delta % 20) <1:
			print(name_space,"Timestamp "+str(delta)+" seconds: travelled distance "+ name_space+ " = " + str(covered_distance));
...
def burgard():
	...
	while not rospy.is_shutdown():
		...
		rospy.sleep(0.5);
		merged_map_lock.acquire();
		print(name_space,"going for frontiers")
		...
		if (len(frontiers)==0):
			print(name_space,"no path to frontiers");
			checking_goals_publisher.publish(Bool(False));
			exit();
		rate = rospy.Rate(0.5);
